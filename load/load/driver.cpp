...
int do_neword (pg_connection &con, file_obj *obj, int t_num, int t_id);

int do_payment(pg_connection &con, file_obj *obj, int t_num, int t_id);

int do_ordstat (pg_connection &con, file_obj *obj, int t_num, int t_id);

int do_delivery(pg_connection &con, file_obj *obj, int t_num, int t_id);

int do_slev (pg_connection &con, file_obj *obj, int t_num, int t_id);
...
int driver(pg_connection &con, int thread_num, file_obj *obj){
    ...
    while((tv1.tv_sec - obj->start.tv_sec) <= total_time){
        switch(seq_get()){
            case 0:
                do_neword(con, obj, thread_num, count);
                break;
            case 1:
                do_payment(con, obj, thread_num, count);
                break;
            case 2:
                do_ordstat(con, obj, thread_num, count);
                break;
            case 3:
                do_delivery(con, obj, thread_num, count);
                break;
            case 4:
                do_slev(con, obj, thread_num, count);
                break;
            default:
...
//              << (void *)do_neword << std::endl;
    ...
    for (i = 0; i < MAX_RETRY; i++) {
        ret = neword(w_id, d_id, c_id, ol_cnt,
                     all_local, itemid, supware, qty, con, obj, t_id);
        if(!ret){
            break;
        }
    }
...
//              << (void *)do_payment << std::endl;
    ...
    for (i = 0; i < MAX_RETRY; i++) {
        ret = payment(w_id, d_id, byname, c_w_id, c_d_id,
                      c_id, c_last, h_amount, con, obj, t_id);
        if(!ret){
            break;
        }
    }
...
//              << (void *)do_ordstat << std::endl;
    ...
    for (i = 0; i < MAX_RETRY; i++) {
        ret = ordstat(w_id, d_id, byname, c_id, c_last, con, obj, t_id);
        if(!ret){
            break;
        }
    }
...
//              << (void *)do_delivery << std::endl;
    ...
    for (i = 0; i < MAX_RETRY; i++) {
        ret = delivery(w_id, o_carrier_id, con, obj, t_id);
        if(!ret){
            break;
        }
    }
...
//              << (void *)do_slev << std::endl;
    ...
    for (i = 0; i < MAX_RETRY; i++) {
        ret = slev(w_id, d_id, level, con, obj, t_id);
        if(!ret){
            break;
        }
    }

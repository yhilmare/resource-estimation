...
int neword(int w_id_arg, int d_id_arg, int c_id_arg,
           int o_ol_cnt_arg, int o_all_local_arg,
           int itemid[], int supware[], int qty[], pg_connection &con, file_obj *obj, int t_id) {
...
//              << (void *)neword << std::endl;
    ...
    try{
         ...
         * "SELECT c_discount, c_last, c_credit, w_tax FROM customer, warehouse WHERE w_id = $1 AND c_w_id = w_id AND c_d_id = $2 AND c_id = $3",
         * */
        std::string sql = "SELECT c_discount, c_last, c_credit, w_tax FROM customer, warehouse WHERE w_id = "
                          + parseInt(w_id) + " AND c_w_id = w_id AND c_d_id = " + parseInt(d_id) + " AND c_id = " + parseInt(c_id);
         ...
         * const parameter_type type1[] = {int_type, int_type};
         * */
        sql = "SELECT d_next_o_id, d_tax FROM district WHERE d_id = " + parseInt(d_id) + " AND d_w_id = "
              + parseInt(w_id) + " FOR UPDATE";
        gettimeofday(&start, NULL);
         ...
         * "UPDATE district SET d_next_o_id = $1 + 1 WHERE d_id = $2 AND d_w_id = $3",
         * */
        sql = "UPDATE district SET d_next_o_id = " + parseInt(d_next_o_id) + " + 1 WHERE d_id = "
              + parseInt(d_id) + " AND d_w_id = " + parseInt(w_id);
        gettimeofday(&start, NULL);
        ...
        sql = "INSERT INTO orders (o_id, o_d_id, o_w_id, o_c_id, o_entry_d, o_ol_cnt, o_all_local) VALUES("
              + parseInt(o_id) + ", " + parseInt(d_id) + ", " + parseInt(w_id) + ", " + parseInt(c_id) + ", "
              + get_pg_date_string(date) + ", " + parseInt(o_ol_cnt) + ", " + parseInt(o_all_local) + ")";
        gettimeofday(&start, NULL);
         ...
         * "INSERT INTO new_orders (no_o_id, no_d_id, no_w_id) VALUES ($1,$2,$3)",
         * */
        sql = "INSERT INTO new_orders (no_o_id, no_d_id, no_w_id) VALUES (" + parseInt(o_id) + ","
              + parseInt(d_id) + "," + parseInt(w_id) + ")";
        gettimeofday(&start, NULL);
        row_count = st.execute_update(sql);
        gettimeofday(&end, NULL);
        interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
        ...
        for (ol_number = 1; ol_number <= o_ol_cnt; ol_number++) {
             ...
             * const parameter_type type5[] = {int_type};
             * */
            sql = "SELECT i_price, i_name, i_data FROM item WHERE i_id = " + parseInt(ol_i_id);
            gettimeofday(&start, NULL);
             ...
             * FROM stock WHERE s_i_id = $1 AND s_w_id = $2 FOR UPDATE",
             * */
            sql = "SELECT s_quantity, s_data, s_dist_01, s_dist_02, s_dist_03, s_dist_04, s_dist_05, s_dist_06, s_dist_07, s_dist_08, s_dist_09, s_dist_10 FROM stock WHERE s_i_id = "
                  + parseInt(ol_i_id) + " AND s_w_id = " + parseInt(ol_supply_w_id) + " FOR UPDATE";
            gettimeofday(&start, NULL);
             ...
             * const parameter_type type7[] = {int_type, int_type, int_type};
             * */
            sql = "UPDATE stock SET s_quantity = " + parseInt(s_quantity) + " WHERE s_i_id = " + parseInt(ol_i_id)
                  + " AND s_w_id = " + parseInt(ol_supply_w_id);
            gettimeofday(&start, NULL);
            row_count = st.execute_update(sql);
            gettimeofday(&end, NULL);
            interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
            ...
            sql = "INSERT INTO order_line (ol_o_id, ol_d_id, ol_w_id, ol_number, ol_i_id, ol_supply_w_id, ol_quantity, ol_amount, ol_dist_info) VALUES ("
                  + parseInt(o_id) + ", " + parseInt(d_id) + ", " + parseInt(w_id) + ", " + parseInt(ol_number) + ", " + parseInt(ol_i_id) + ", "
                  + parseInt(ol_supply_w_id) + ", " + parseInt(ol_quantity) + ", " + tmp_float + ", '" + std::string(ol_dist_info) + "')";
            gettimeofday(&start, NULL);
            row_count = st.execute_update(sql);
            gettimeofday(&end, NULL);
            interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);

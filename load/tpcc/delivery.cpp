...
//              << (void *)delivery << std::endl;
    ...
    try{
        ...
        for (d_id = 1; d_id <= DISTRICT_PER_WAREHOUSE; d_id++) {
            ...
            std::string sql = "SELECT COALESCE(MIN(no_o_id),0) FROM new_orders WHERE no_d_id = " + parseInt(d_id)
                              + " AND no_w_id = " + parseInt(w_id);
            timeval start;
            gettimeofday(&start, NULL);
...
//                      << (void *)&st << std::endl;
             ...
             * const parameter_type type26[] = {int_type, int_type, int_type};
             * */
            sql = "DELETE FROM new_orders WHERE no_o_id = " + parseInt(no_o_id) + " AND no_d_id = " + parseInt(d_id)
                  + " AND no_w_id = " + parseInt(w_id);
            gettimeofday(&start, NULL);
...
//                      << (void *)&st1 << std::endl;
             ...
             * "SELECT o_c_id FROM orders WHERE o_id = $1 AND o_d_id = $2 AND o_w_id = $3",
             * */
            sql = "SELECT o_c_id FROM orders WHERE o_id = " + parseInt(no_o_id) + " AND o_d_id = " + parseInt(d_id)
                  + " AND o_w_id = " + parseInt(w_id);
            gettimeofday(&start, NULL);
...
//                      << (void *)&st2 << std::endl;
             ...
             * const parameter_type type28[] = {int_type, int_type, int_type, int_type};
             * */
            sql = "UPDATE orders SET o_carrier_id = " + parseInt(o_carrier_id) + " WHERE o_id = " + parseInt(no_o_id)
                  + " AND o_d_id = " + parseInt(d_id) + " AND o_w_id = " + parseInt(w_id);
            gettimeofday(&start, NULL);
...
//                      << (void *)&st3 << std::endl;
            ...
            PG::Date date(seconds - d3(e));
            sql = "UPDATE order_line SET ol_delivery_d = " + get_pg_date_string(date) + " WHERE ol_o_id = "
                  + parseInt(no_o_id) + " AND ol_d_id = " + parseInt(d_id) + " AND ol_w_id = " + parseInt(w_id);
            gettimeofday(&start, NULL);
            row_count = st.execute_update(sql);
            gettimeofday(&end, NULL);
            interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
...
//                      << (void *)&st4 << std::endl;
             ...
             * const parameter_type type30[] = {int_type, int_type, int_type};
             * */
            sql = "SELECT SUM(ol_amount) FROM order_line WHERE ol_o_id = " + parseInt(no_o_id) + " AND ol_d_id = "
                  + parseInt(d_id) + " AND ol_w_id = " + parseInt(w_id);
            gettimeofday(&start, NULL);
...
//                      << (void *)&st5 << std::endl;
             ...
             * */
            char buffer[100];
            sprintf(buffer, "%.5f", ol_total);
            std::string tmp_float = std::string(buffer);
            sql = "UPDATE customer SET c_balance = c_balance + " + tmp_float + " , c_delivery_cnt = c_delivery_cnt + 1 WHERE c_id = "
                  + parseInt(c_id) + " AND c_d_id = " + parseInt(d_id) + " AND c_w_id = " + parseInt(w_id);
            gettimeofday(&start, NULL);
            row_count = st.execute_update(sql);
            gettimeofday(&end, NULL);
            tran_obj.add_item(transaction_item(EXCLUSIVE_LOCK, "customer",

...
int payment(int w_id_arg, int d_id_arg, int byname,
            int c_w_id_arg, int c_d_id_arg, int c_id_arg,
            char c_last_arg[], float h_amount_arg, pg_connection &con, file_obj *obj, int t_id) {
...
//              << (void *)payment << std::endl;
    ...
    try{
         ...
         * */
        char buffer[100];
        sprintf(buffer, "%.5f", h_amount);
        std::string tmp_float = std::string(buffer);
        std::string sql = "UPDATE warehouse SET w_ytd = w_ytd + " + tmp_float + " WHERE w_id = " + parseInt(w_id);
        pg_statement st = con.create_statement();
        timeval start;
        gettimeofday(&start, NULL);
        int row_count = st.execute_update(sql);
        timeval end;
        gettimeofday(&end, NULL);
        long interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
        long series = (start.tv_sec * 1000000 + start.tv_usec) - (obj->start.tv_sec * 1000000 + obj->start.tv_usec);
...
//                  << (void *)&st << std::endl;
         ...
         * const parameter_type type10[] = {int_type};
         * */
        sql = "SELECT w_street_1, w_street_2, w_city, w_state, w_zip, w_name FROM warehouse WHERE w_id = " + parseInt(w_id);
        gettimeofday(&start, NULL);
...
//                  << (void *)&st1 << std::endl;
         ...
         * */
        char buffer1[100];
        sprintf(buffer1, "%.5f", h_amount);
        std::string tmp_float1 = std::string(buffer1);
        sql = "UPDATE district SET d_ytd = d_ytd + " + tmp_float1 + " WHERE d_w_id = " + parseInt(w_id) + " AND d_id = " + parseInt(d_id);
        gettimeofday(&start, NULL);
...
//                  << (void *)&st2 << std::endl;
         ...
         * "SELECT d_street_1, d_street_2, d_city, d_state, d_zip, d_name FROM district WHERE d_w_id = $1 AND d_id = $2",
         * */
        sql = "SELECT d_street_1, d_street_2, d_city, d_state, d_zip, d_name FROM district WHERE d_w_id = "
              + parseInt(w_id) + " AND d_id = " + parseInt(d_id);
        gettimeofday(&start, NULL);
...
//                  << (void *)&st3 << std::endl;
        if (byname) {
             ...
             * const parameter_type type13[] = {int_type, int_type, text_type};
             * */
            sql = "SELECT count(c_id) FROM customer WHERE c_w_id = " + parseInt(c_w_id) + " AND c_d_id = "
...
//                      << (void *)&st4 << std::endl;
             ...
             * "SELECT c_id FROM customer WHERE c_w_id = $1 AND c_d_id = $2 AND c_last = $3 ORDER BY c_first",
             * */
            sql = "SELECT c_id FROM customer WHERE c_w_id = " + parseInt(c_w_id) + " AND c_d_id = "
                  + parseInt(c_d_id) + " AND c_last = '" + std::string(c_last) + "' ORDER BY c_first";
            gettimeofday(&start, NULL);
...
//                      << (void *)&st5 << std::endl;
         ...
         * const parameter_type type15[] = {int_type, int_type, int_type};
         * */
        sql = "SELECT c_first, c_middle, c_last, c_street_1, c_street_2, c_city, c_state, c_zip, c_phone, c_credit, c_credit_lim, c_discount, c_balance, c_since FROM customer WHERE c_w_id = "
              + parseInt(c_w_id) + " AND c_d_id = " + parseInt(c_d_id) + " AND c_id = " + parseInt(c_id) + " FOR UPDATE";
        gettimeofday(&start, NULL);
...
//                  << (void *)&st6 << std::endl;
        ...
        if (strstr(c_credit, "BC")) {
             ...
             * "SELECT c_data FROM customer WHERE c_w_id = $1 AND c_d_id = $2 AND c_id = $3",
             * */
            sql = "SELECT c_data FROM customer WHERE c_w_id = " + parseInt(c_w_id) + " AND c_d_id = "
                  + parseInt(c_d_id) + " AND c_id = " + parseInt(c_id);
            gettimeofday(&start, NULL);
            pg_resultset res5 = st.execute_query(sql);
            gettimeofday(&end, NULL);
            interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
            series = (start.tv_sec * 1000000 + start.tv_usec) - (obj->start.tv_sec * 1000000 + obj->start.tv_usec);
            while(res5.has_next()){
...
//                      << (void *)&st7 << std::endl;
             ...
             * */
            char buffer2[100];
            sprintf(buffer2, "%.5f", c_balance);
            std::string tmp_float2 = std::string(buffer2);
            sql = "UPDATE customer SET c_balance = " + tmp_float2 + ", c_data = '" + std::string(c_data)
                  + "' WHERE c_w_id = " + parseInt(c_w_id) + " AND c_d_id = " + parseInt(c_d_id) + " AND c_id = " + parseInt(c_id);
            gettimeofday(&start, NULL);
            row_count = st.execute_update(sql);
            gettimeofday(&end, NULL);
            interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
...
//                      << (void *)&st8 << std::endl;
             ...
             * */
            char buffer3[100];
            sprintf(buffer3, "%.5f", c_balance);
            std::string tmp_float3 = std::string(buffer3);
            sql = "UPDATE customer SET c_balance = " + tmp_float3 + " WHERE c_w_id = " + parseInt(c_w_id)
                  + " AND c_d_id = " + parseInt(c_d_id) + " AND c_id = " + parseInt(c_id);
            gettimeofday(&start, NULL);
            row_count = st.execute_update(sql);
            gettimeofday(&end, NULL);
            interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
...
//                      << (void *)&st9 << std::endl;
         ...
         * */
        char buffer4[100];
        sprintf(buffer4, "%.5f", h_amount);
        std::string tmp_float4 = std::string(buffer4);
        PG::Date date(seconds - d3(e));
        sql = "INSERT INTO history(h_c_d_id, h_c_w_id, h_c_id, h_d_id, h_w_id, h_date, h_amount, h_data) VALUES("
              + parseInt(c_d_id) + ", " + parseInt(c_w_id) + ", " + parseInt(c_id) + ", " + parseInt(d_id) + ", "
              + parseInt(w_id) + ", " + get_pg_date_string(date) + ", " + tmp_float4 + ", '" + std::string(h_data) + "')";
        gettimeofday(&start, NULL);
        row_count = st.execute_update(sql);
        gettimeofday(&end, NULL);
        interval = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
